...
\begin{abstract}
Every wildfire prediction model deployed today shares a dangerous property: none of them guarantee how many fires they will miss.
...
We expose a stark failure: across three model families of increasing complexity---tabular (LightGBM, AUROC 0.854), convolutional (Tiny U-Net, AUROC 0.969), and graph-based (Hybrid ResGNN-UNet, AUROC 0.951)---standard thresholds capture only 7--52\% of true fire pixels.
CRC eliminates this failure uniformly.
Our central finding is that \emph{model architecture determines evacuation efficiency, while CRC determines safety}: a 470K-parameter U-Net with CRC achieves 94.7\% fire coverage while flagging only 14.9\% of pixels---4.2$\times$ more efficient than LightGBM---while the architecturally more complex 229K-parameter graph model achieves lower AUROC.
...
At their respective optimal thresholds, LightGBM misses 92.8\% of fires, U-Net misses 61.0\%, and the ResGNN-UNet misses 48.5\%.
With CRC, the simplest spatial model achieves 94.7\% coverage while flagging only 14.9\% of pixels---outperforming the architecturally more complex graph model in both safety and efficiency.
...
\begin{itemize}
\item We demonstrate that standard probability thresholding is \emph{catastrophically unsafe} for wildfire evacuation across three model families, with fire coverage ranging from 7\% to 52\%.
...
\item We provide a complete, reproducible pipeline with deterministic splits and open-source code for all experimental phases.
\end{itemize}
...
CRC has since been applied to medical imaging, autonomous driving, and natural language processing---but not to any geospatial or environmental disaster domain.
...
\begin{table}[t]
\caption{Test-set results across three models and three decision frameworks.  CRC targets $\mathrm{FNR} \le 0.05$.  Three-way CRC uses $c_{\mathrm{fn}}{=}5$, $c_{\mathrm{fp}}{=}1$, $\rho \in [0.9, 1.1]$.  ResGNN-UNet rows report recall at fixed thresholds; CRC was not applied because the model was trained externally and its calibration-set probabilities were not available in our reproducible pipeline.}
...
\midrule
LGBM ($p{\ge}.5$) & --- & .500 & .072 & .928 & .001 & .854 \\
LGBM+CRC & --- & .003 & .941 & .059 & .626 & .854 \\
LGBM+3way & --- & {\scriptsize[0,.03]} & 1.00 & .000 & .040 & .854 \\
\midrule
UNet ($p{\ge}.5$) & 470K & .500 & .390 & .610 & .007 & .969 \\
\textbf{UNet+CRC} & \textbf{470K} & \textbf{.002} & \textbf{.947} & \textbf{.053} & \textbf{.149} & \textbf{.969} \\
UNet+3way & 470K & {\scriptsize[0,.02]} & 1.00 & .000 & .051 & .969 \\
\midrule
ResGNN ($p{\ge}.5$) & 229K & .500 & .515\textsuperscript{$\dagger$} & .485 & --- & .951 \\
ResGNN ($p{\ge}.6$) & 229K & .600 & .342\textsuperscript{$\dagger$} & .658 & --- & .951 \\
\bottomrule
\multicolumn{7}{l}{\scriptsize $\dagger$Recall at F1-optimal threshold on validation set.}
\end{tabular}
\end{table}
...
At $\hat{p} \ge 0.5$, LightGBM captures only 7.2\% of fire pixels, U-Net 39.0\%, and ResGNN-UNet 51.5\%.
Even at the ResGNN-UNet's F1-optimal threshold ($\hat{p} \ge 0.6$), 65.8\% of fires are missed.
The ${\sim}95\%$ non-fire class dominance concentrates probability mass near zero, making any conventional threshold catastrophically unsafe (\Cref{fig:cross-section}).
...
CRC raises both primary models to ${\sim}95\%$ coverage, meeting the $\mathrm{FNR} \le 0.05$ target.
However, the cost differs substantially (\Cref{fig:three-model}):
LightGBM must flag 62.6\% of all pixels, while U-Net flags only 14.9\%---a \textbf{4.2$\times$ reduction} in evacuation zone size.
This gap is explained by the AUROC difference (0.854 vs.\ 0.969): better fire/non-fire separation allows CRC to set a higher $\hat{\lambda}$ while maintaining the same safety guarantee.
\Cref{fig:safety-efficiency} (Appendix) visualizes this safety--efficiency decomposition across all six configurations.

\paragraph{Architectural complexity $\ne$ better separation.}
The ResGNN-UNet achieves lower AUROC (0.951) than the Tiny U-Net (0.969) despite its more complex architecture (graph attention bottleneck).
On $64{\times}64$ patches, the U-Net's receptive field already covers the full spatial extent; the GNN bottleneck adds overhead without improving separation.
This reinforces our central claim: \emph{a simple model with CRC outperforms a complex model without it}.
...
Without CRC, the standard threshold produces predominantly red error maps---vast regions of missed fire.
...
\caption{The safety gap across three test samples (0.5\%, 2.7\%, 7.8\% fire).  \textbf{Col.\,1:} ground truth.  \textbf{Col.\,2:} standard threshold---missed fire in red (100\%, 73\%, 34\% missed).  \textbf{Col.\,3:} CRC threshold---improved detection (green).  \textbf{Col.\,4:} three-way zones (SAFE/MONITOR/EVACUATE).}
...
ResGNN-UNet & 229K & 12\textsuperscript{$\dagger$} & AdamW & 0.170 & .951 \\
\bottomrule
\multicolumn{6}{l}{\scriptsize $\dagger$Early-stopped from 25 (patience=7, best epoch 5).}
\end{tabular}
\end{table}
...
The ResGNN-UNet early-stopped at epoch 12 (best epoch 5, val loss 0.170 vs.\ train loss 0.12), exhibiting overfitting that suggests the graph attention bottleneck does not generalize well at this patch scale.
...
All models achieve the same FNR guarantee; what varies is the evacuation zone size.
This decomposition has practical significance: teams can invest in better models to reduce evacuation costs without compromising or re-validating safety properties.

\paragraph{Diminishing returns from complexity.}
The ResGNN-UNet, despite its graph attention bottleneck designed to model fire propagation, achieves lower AUROC than the Tiny U-Net.
On $64{\times}64$ patches, the U-Net's receptive field spans the full tile; GNN message-passing over an 8-connected grid provides no information that spatial convolutions cannot capture.
This echoes broader findings that inductive bias alignment matters more than raw model capacity \citep{ronneberger2015unet}.
...
CRC's guarantee requires exchangeability between calibration and test data, which may be violated under distributional shift (e.g., novel fire regimes, climate change).
Our analysis uses a single dataset (NDWS) with US wildfire events; generalization to other geographies requires further validation.
...
An image-level formulation---defining the loss as per-image FNR and applying CRC to the $n{=}2{,}781$ image-level losses---would restore theoretical validity at the cost of a smaller effective sample size.
...
All code, trained models, and evaluation pipelines are maintained in a reproducible repository with deterministic data splits and CLI entry points for each phase.
...
\caption{Safety vs.\ efficiency for all six configurations.  \textbf{Top:} coverage (safety)---only CRC and three-way methods achieve ${\ge}95\%$.  \textbf{Bottom:} set size (efficiency)---U-Net+CRC is the optimal operating point.}
